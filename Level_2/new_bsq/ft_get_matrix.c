...
int		ft_get_line(t_map *map, int i, FILE *file)
{
	int		j;
	int		buffer;
	...
	while (1)
	{
		...
		// If the char is different from empty and obstacle, returns True
		if (buffer != map->empty && buffer != map->obstacle)
			return (1);
...
int		ft_get_first_line(t_map *map, FILE *file)
{
	int			buffer;
	int			i;
...
int				ft_get_matrix(t_map *map, FILE *file)
{
	int		i;
	int		buffer;
